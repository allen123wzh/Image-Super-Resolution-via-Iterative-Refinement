...
class GlobalCorrector(nn.Module):
    ...
    def forward(self, x, time):
        ...
        if self.normal01:
            x = (x + 1) / 2
        cond = self.cond_net(x)         # [N, cond_nf]

        scale1 = self.cond_scale1(cond) # [N, base_nf]
        shift1 = self.cond_shift1(cond) # [N, base_nf]
        ...
        out = self.conv2(out)       # [N, base_nf, H, W]
        out = self.convt2(out, t)   # [N, base_nf, H, W]
        out = out * scale2.view(-1, self.base_nf, 1, 1) + shift2.view(-1, self.base_nf, 1, 1) + out
        out = self.act(out)

        out = self.conv3(out)       # [N, out_nc, H, W]
        out = self.convt3(out, t)   # [N, out_nc, H, W]
